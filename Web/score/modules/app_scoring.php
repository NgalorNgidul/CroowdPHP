<?php
  ...
  function daftar_aplikasiscoring($cmd){
     global $_STATUS, $COOK_USER_ID, $_MAX_REC_PER_PAGE, $_ScoringType, $_ApplicationType, $_JPERMOHONAN, $COOK_USERUNIT, $_USER, $_UNITNAME, $COOK_USER_GROUP_ID;
     ...
     $tgrid->set_content(array("<b>Application List</b>"));
     $isi .= $tgrid->build();
     #print $COOK_USER_ID;

     //if($COOK_USER_ID!="1" && $COOK_USER_ID!="4" && $COOK_USER_ID!="5")
     if($COOK_USER_GROUP_ID!="1" && $COOK_USER_GROUP_ID!="20" && $COOK_USER_GROUP_ID!="21")
        $where .= "AND APPLICATION_CABANG = '$COOK_USERUNIT'";

     $where .= " AND STATUS <> 99";
     $sql    = " SELECT * FROM BUS_APPLICATION WHERE 1=1 $where";
     ...
     $sql_pagging = " SELECT TOP $_MAX_REC_PER_PAGE * FROM BUS_APPLICATION WHERE 1=1 $where AND ID_BUS_APPLICATION NOT IN(SELECT  TOP $curr_page  ID_BUS_APPLICATION FROM BUS_APPLICATION WHERE 1=1 $where)";
     #print $sql;
     ...
     while($data=sqlsrv_fetch_array($query_pagging)){
        ...
        $detail_ver= "<a href=\"index.php?cmd=".$cmd."_VER_detail&case=1&xid=".$data[ID_BUS_APPLICATION]."\"><img src=\"includes/page_main/img/detail.png\" border=\"0\" width=\"15\"></a>";
        $cetak_ver= "<a href=\"index.php?cmd=".$cmd."_VER_cetak&case=1&xid=".$data[ID_BUS_APPLICATION]."\">Result</a>";
        $sent= "<a href=\"index.php?cmd=".$cmd."_sent&case=1&xid=".$data[ID_BUS_APPLICATION]."\"><img src=\"includes/page_main/img/Send_mail.png\" border=\"0\" width=\"15\"></a>";
        ...
        $tgrid->set_content_align(array("center","left","left","left","left","left","center","left","left"));
        $tgrid->set_content_width(array("5","180","","200","200","","","","100"));
        $tgrid->set_content(array("$x",$_ScoringType[$data[ID_SCORING_TYPE]]."&nbsp;",$data[APPLICATION_NUMBER]."&nbsp;", $_USER[$data[APPLICATION_AONAME]]."&nbsp;", $_UNITNAME[$data[APPLICATION_CABANG]]."&nbsp;", $data[APPLICATION_CIFNAME]."&nbsp;", date("d-M-Y", strtotime($data["APPLICATION_DATE"]))."&nbsp;", $ket."&nbsp;", ($data['STATUS']>="3"?$detail_ver."&nbsp;".$cetak_ver:$detail)."&nbsp;".($data[STATUS]<="2"?$edit:"&nbsp;")."&nbsp;".($data[STATUS]<="2"?$delete:"&nbsp;").($data[STATUS]<="2"?$sent:"&nbsp;")));
        $x++;
     }
     ...
     return  Template_KotakPolos("Application List",$isi);
  }

   function frm_application($cmd){
      ...
      if($_REQUEST[xid]!=""){
        $data            = module_getApplication_byid($_REQUEST[xid]);
      ...
      if($status!="2" && $status!="99"){
          ...
          $in         = array("APP_NUMBER"=>$nomor,"STATUS"=>"0000","CREATED_BY"=>$aoname);
          insert_log_app_number($in);
      ...
      return Template_KotakPolos("Application",$form);
  }

   function view_application($cmd){
  ...
  function generate_NumberApp(){
      $head   = date("my");
      $sql    = " SELECT MAX(CAST(APPLICATION_NUMBER AS INT)) AS APPLICATION_NUMBER FROM BUS_APPLICATION WHERE LEFT(APPLICATION_NUMBER,4)=$head";
      #print $sql;
      GLOBAL $connectionInfo, $SQLHost;
      $conn = sqlsrv_connect( $SQLHost, $connectionInfo);
      $query = sqlsrv_query($conn,$sql);
      ...
      $dig    = substr($data[APPLICATION_NUMBER],"3");
      #print $data[APPLICATION_NUMBER];
      $hsl    = ($data[APPLICATION_NUMBER]!=""?$head.($dig+1):$head."1");
      $sql_log  = " SELECT * FROM LOG_APP_NUMBER WHERE APP_NUMBER = ".$hsl;
      //print $sql_log;
      $query_log = sqlsrv_query($conn,$sql_log);
      $data_log   = sqlsrv_fetch_array($query_log);
      $hsl        = ($data_log['APP_NUMBER']!=""?$data_log['APP_NUMBER']+1:$hsl);
      $cek_log    = cek_log($hsl);//print $data_log['APP_NUMBER'];
      return $cek_log;
  }

  function cek_log($app_number){
      $sql  = " SELECT MAX(APP_NUMBER) AS APP_NUMBER FROM LOG_APP_NUMBER";
      //print $sql_log;
      GLOBAL $connectionInfo, $SQLHost;
      $conn = sqlsrv_connect( $SQLHost, $connectionInfo);
      $query = sqlsrv_query($conn,$sql);
      ...
      $hsl        = ($data['APP_NUMBER']<=$app_number?$app_number+1:$data['APP_NUMBER']+1);
      //print_r($data['APP_NUMBER']."##".$app_number);
      return $hsl;
  }

  function application_save($cmd){
      ...
      if($app_cifname!="" && $app_number!=""){
          $id            = getId("BUS_APPLICATION","ID_BUS_APPLICATION");
          $sql           = "  INSERT INTO BUS_APPLICATION(ID_BUS_APPLICATION,ID_SCORING_TYPE,APPLICATION_NUMBER,
                                                          APPLICATION_AONAME,APPLICATION_CABANG,APPLICATION_CIFNO,
                                                          APPLICATION_DATE,APPLICATION_JENIS,
                                                          CREATED_BY,CREATED_TIMESTAMP,STATUS,
                                                          APPLICATION_CIFNAME, APPLICATION_BADANHUKUM, APPLICATION_USIA,
                                                          APPLICATION_PENDIDIKAN, APPLICATION_TELEPON, APPLICATION_JUSAHA,
                                                          APPLICATION_JMLKARYAWAN, APPLICATION_ALAMAT, APPLICATION_LUSAHA,
                                                          APPLICATION_LAMAHUB, APPLICATION_OTHERFINANCE, APPLICATION_TENOR,
                                                          APPLICATION_PLAFOND, APPLICATION_LAGUNAN, APPLICATION_COLCOV)
                              VALUES('$id','$scoringtype','$app_number',
                                     '$app_aoname','$app_cabang','$app_cif_no',
                                     '$app_date','$app_jenis',
                                     '1',GETDATE(),'1',
                                     '$app_cifname','$app_badanusaha','$app_usia',
                                     '$app_pendidikan','$app_telepon','$app_jusaha',
                                     '$app_jkaryawan','$app_alamat','$app_lusaha',
                                     '$app_lhub','$app_otherfinance','$app_tenor',
                                     '$app_plafond','$app_nlikuidasi','$app_colcov'
                                     )";
          //print $sql; exit();
          GLOBAL $connectionInfo, $SQLHost;
          $conn = sqlsrv_connect( $SQLHost, $connectionInfo);
          ...
          if($query==1){
             $sql_update_nomor  = " UPDATE LOG_APP_NUMBER SET STATUS = '0001' WHERE APP_NUMBER = '".$app_number."' ";
             //$query_update_nomor= mssql_query($sql_update_nomor);
             $query_update_nomor = sqlsrv_query($conn,$sql_update_nomor);           
             return array("xid"=>$id,"msg"=>Template_KotakPolos("Aplikasi",$msgDBSave));
          }else{
             return array("xid"=>$id,"msg"=>Template_KotakPolos("Aplikasi",$msgDBError));
          }
      }else{
          return array("xid"=>$id,"msg"=>Template_KotakPolos("Aplikasi",$msgDBError."<br>..::: Lengkapi Nama Pemohon :::.."));
      }
  }

  function module_getApplication_byid($id){
      $sql  = " SELECT * FROM BUS_APPLICATION WHERE ID_BUS_APPLICATION='$id'";
      GLOBAL $connectionInfo, $SQLHost;
      $conn = sqlsrv_connect( $SQLHost, $connectionInfo);
      $query = sqlsrv_query($conn,$sql);
  ...
  function application_update($cmd){
      global  $msgDBSave,  $msgDBError;
      $scoringtype           = $_REQUEST[scoringtype];
      $app_number            = $_REQUEST[nomor];
      $app_aoname            = $_REQUEST[aoname];
      ...
      $app_otherfinance      = $_REQUEST[otherfinancing];
      $id            = $_REQUEST[xid];
      $sql           = "  UPDATE BUS_APPLICATION SET  ID_SCORING_TYPE='$scoringtype',
      ...
      $query = sqlsrv_query($conn,$sql);      
      if($query){
         return Template_KotakPolos("Aplikasi",$msgDBSave);
      }else{
         return Template_KotakPolos("Aplikasi",$msgDBError);
      }
  }

 function daftar_appcollateral($cmd){
     global $_STATUS, $COOK_USER_ID, $_MAX_REC_PER_PAGE, $_CollateralType, $_CollType;
     $app  = module_getApplication_byid($_REQUEST[xid]);
     ...
     $sql    = " SELECT * FROM BUS_COLLATERAL WHERE 1=1 $where";
     GLOBAL $connectionInfo, $SQLHost;
     $conn = sqlsrv_connect( $SQLHost, $connectionInfo);
     $query = sqlsrv_query($conn,$sql);
     $jml_record  = sqlsrv_num_rows($query);
     if($_GET[page] < 1)	$_GET[page] = 1;
  	   $curr_page = ($_GET[page] - 1) * $_MAX_REC_PER_PAGE;
     $sql_pagging = " SELECT TOP $_MAX_REC_PER_PAGE * FROM BUS_COLLATERAL WHERE 1=1 $where AND ID_BUS_COLL NOT IN(SELECT  TOP $curr_page  ID_BUS_COLL FROM BUS_COLLATERAL WHERE 1=1 $where)";
     #print $sql_pagging;
     ...
     $update_colcov = module_updatecolcov($_REQUEST[xid], $colcov, $total);
 ...
 function frm_collapplication($cmd){
      global $_CollType;
      $app  = module_getApplication_byid($_REQUEST[xid]);
 ...
 function bus_collsave($cmd){
    ...
    $cek_jcoll        = cek_jcoll($id_app, '0001');
    GLOBAL $connectionInfo, $SQLHost;
    $conn = sqlsrv_connect( $SQLHost, $connectionInfo); 
       
    if($jcoll=="0001"){
      if($cek_jcoll<=0){
          $sql           = "  INSERT INTO BUS_COLLATERAL(ID_BUS_COLL,ID_COLL,ID_BUS_APPLICATION,COLL_VALUE,STATUS, COLL_TYPE)
 ...
 function cek_jcoll($xid, $jcoll){
      $sql    = " SELECT * FROM BUS_COLLATERAL WHERE ID_BUS_APPLICATION='$xid' AND COLL_TYPE='$jcoll' AND STATUS <>'99'";
      GLOBAL $connectionInfo, $SQLHost;
      $conn = sqlsrv_connect( $SQLHost, $connectionInfo);
      $query = sqlsrv_query($conn,$sql);
...
*/
 ...
 function  get_frmcriteria_save($cmd){
    ...
    foreach($criteria as $cparents=>$vparents){
      foreach($vparents as $ckey=>$cvalue){
         $yatidak  = $dataCriteria[$_REQUEST[xid]][$ckey]["BUS_ACCEPTANCE_CRITERIA"];
         $ket      = $dataCriteria[$_REQUEST[xid]][$ckey]["BUS_ACCEPTANCE_KET"];
         $id       = getId("BUS_ACCEPTANCE_CRITERIA","ID_BUS_ACCEPTANCE");
         if(count($yatidak)=="0" && count($ket)=="0"){
           $sql      = " INSERT INTO BUS_ACCEPTANCE_CRITERIA (ID_BUS_ACCEPTANCE,ID_BUS_APPLICATION,ID_CRITERIA,BUS_ACCEPTANCE_CRITERIA,BUS_ACCEPTANCE_KET)
                         VALUES('$id','$_REQUEST[xid]','$ckey','".$_REQUEST["yatidak_".$ckey]."','".$_REQUEST["ket_".$ckey]."')";
            GLOBAL $connectionInfo, $SQLHost;
            $conn = sqlsrv_connect( $SQLHost, $connectionInfo);
            $query = sqlsrv_query($conn,$sql);
         }else{
           $sql = " UPDATE BUS_ACCEPTANCE_CRITERIA SET BUS_ACCEPTANCE_CRITERIA='".$_REQUEST["yatidak_".$ckey]."',
 ...
 function  get_frmcriteria_update($cmd){
 ...
 function view_frmneraca_($cmd){
    ...
    foreach($criteria as $cparents=>$vparents){
      foreach($vparents as $ckey=>$cvalue){
        ...
        $ketawal          = trim($dataCriteria[$_REQUEST[xid]][$ckey]["BUS_NERACA_KETAWAL"]);
        $ketakhir         = trim($dataCriteria[$_REQUEST[xid]][$ckey]["BUS_NERACA_KETAKHIR"]);
        $ketjalan         = trim($dataCriteria[$_REQUEST[xid]][$ckey]["BUS_NERACA_KETJALAN"]);
        $tahunawal        = $dataCriteria[$_REQUEST[xid]][$ckey]["BUS_NERACA_TAHUNAWAL"];
        $tahunakhir       = $dataCriteria[$_REQUEST[xid]][$ckey]["BUS_NERACA_TAHUNAKHIR"];
        $tahunjalan       = $dataCriteria[$_REQUEST[xid]][$ckey]["BUS_NERACA_TAHUNJALAN"];
        $monjalan         = trim($dataCriteria[$_REQUEST[xid]][$ckey]["BUS_NERACA_MONJALAN"]);
    ...
    $tahun  = "<table width=\"100%\" border=\"0\">";
    $tahun  .= "<tr>";
    $tahun  .= "<td width=\"200\" align=\"right\" class=\"\"><b>".$tahunawal."</b></td>";
    $tahun  .= "<td width=\"200\" align=\"right\" class=\"\"><b>".$tahunakhir."</b></td>";
    $tahun  .= "<td width=\"200\" align=\"right\" class=\"\"><b>".$tahunjalan."</b></td>";
    $tahun  .= "</tr>";
    $tahun  .= "<tr>";
    $tahun  .= "<td width=\"200\" align=\"right\"><b>".$_NERACAKET[$ketawal]."</b></td>";
    $tahun  .= "<td width=\"200\" align=\"right\"><b>".$_NERACAKET[$ketakhir]."</b></td>";
    $tahun  .= "<td width=\"200\" align=\"right\"><b>".$_Month[$monjalan]."</b></td>";
    $tahun  .= "</tr>";
    $tahun  .= "</table>";
    $cf->set_row($row_id,$ext=array("class"	=> "tablecontent"));
    $cf->set_col_header($row_id,$text="",$ext=array("width"=>"150","class"	=> "tablecontentgrid","valign"=>"top"));
    $cf->set_col_content($row_id,$tahun,$ext=array("valign"=>"top", "class"	=> "tablecontentgrid"));
    $row_id++;
    ...
    foreach($criteria as $cparents=>$vparents){
      ...
      $tahun  = "&nbsp;";
      $cf->set_row($row_id,$ext=array("class"	=> "tablecontent"));
      $cf->set_col_header($row_id,$text=$input,$ext=array("width"=>"150","class"	=> "tableheader","valign"=>"top"));
      $cf->set_col_content($row_id,$tahun,$ext=array("valign"=>"top", "class"	=> "tableheader"));
      $row_id++;
      foreach($vparents as $ckey=>$cvalue){
        ...
        $tahunakhir       = $dataCriteria[$_REQUEST[xid]][$ckey]["BUS_NERACA_TAHUNAKHIR"];
        $tahunjalan       = $dataCriteria[$_REQUEST[xid]][$ckey]["BUS_NERACA_TAHUNJALAN"];
        $nilaiawal        = $dataCriteria[$_REQUEST[xid]][$ckey]["BUS_NERACA_VALUEAWAL"];
        $nilaiakhir       = $dataCriteria[$_REQUEST[xid]][$ckey]["BUS_NERACA_VALUEAKHIR"];
        $nilaijalan       = $dataCriteria[$_REQUEST[xid]][$ckey]["BUS_NERACA_VALUEJALAN"];
        #print $ket;
        #$input	 = "<table><tr><td width=\"200\">".inputSelect($name="ket_".$ckey,$_NERACAKET,$ket, $ext=array("suffix"=>"Ket<br>","onkeypress"=> "return disableEnter(this,event);"))."</td></tr></table>";
        #$input	 .= "<table><tr><td width=\"200\">".inputSelectRange($name="tahunawal_".$ckey,date("Y")-5,date("Y")+5,$tahunawal, $ext=array("suffix"=>"Tahun Sebelumnya &nbsp;","onkeypress"=> "return disableEnter(this,event);"));
        #$input	 .= "</td><td>".inputText($name="nilaiawal_".$ckey,$nilaiawal, $ext=array("suffix"=>"&nbsp;Nilai Sebelumnya<br>","onkeypress"=> "return disableEnter(this,event);"))."</td></tr></table>";
        ...
        $input  = "<table width=\"100%\" border=\"0\">";
        $input  .= "<tr>";
        $input  .= "<td width=\"200\" align=\"right\" class=\"\">".number_format($nilaiawal)."</td>";
        $input  .= "<td width=\"200\" align=\"right\" class=\"\">".number_format($nilaiakhir)."</td>";
        $input  .= "<td width=\"200\" align=\"right\" class=\"\">".number_format($nilaijalan)."</td>";
        $input  .= "</tr>";
        $input  .= "</table>";

        $cf->set_row($row_id,$ext=array("class"	=> "tablecontent"));
        $cf->set_col_header($row_id,$text=$cvalue,$ext=array("width"=>"150","class"	=> "tablecontentgrid","valign"=>"top"));
        $cf->set_col_content($row_id,$input,$ext=array("valign"=>"top", "class"	=> "tablecontentgrid",));
        $row_id++;
      }
  ...
  function view_frmneraca($cmd){
     ...
     $tgrid->set_content(array("","",generate_tree_listneraca_view($data_r,"0","0",$cmd)));
     #$isi .= $tgrid->set_nav($jml_record,"$_GET[page]");
     $isi .= $tgrid->build();
     $isi   .= "</div>";
  ...
  function generate_tree_listneraca_view($array, $parent = "0", $level = "0", $cmd)
  {
    ...
    $dt = generate_tree_listneraca_test($new_array, "0", "0", $cmd, $asap);
    return $dt;

  }

  function generate_tree_listneraca_test($array, $parent = "0", $level = "0", $cmd, $total_value)
  {
    global  $_NeracaType, $_NERACAKET, $_Month;
    $dataCriteria = get_NeracaByApp($_REQUEST[xid]);
    #print_r($array); exit();
    ...
    { //print $value[PARENT_NERACA]."###".$key."<br>";
      if ($value['PARENT_NERACA'] == $parent)
      {
        ...
        $dt .= generate_tree_listneraca_test($array, $key, $level, $cmd, $total_value);
 ...
 function  get_frmneraca_save_($cmd){
    ...
    foreach($criteria as $cparents=>$vparents){
      foreach($vparents as $ckey=>$cvalue){
         ...
         $ketakhir         = $dataCriteria[$_REQUEST[xid]][$ckey]["BUS_NERACA_KETAKHIR"];
         $ketjalan         = $dataCriteria[$_REQUEST[xid]][$ckey]["BUS_NERACA_KETJALAN"];
         $tahunawal[$ckey]        = $dataCriteria[$_REQUEST[xid]][$ckey]["BUS_NERACA_TAHUNAWAL"];
         $tahunakhir       = $dataCriteria[$_REQUEST[xid]][$ckey]["BUS_NERACA_TAHUNAKHIR"];
         $tahunjalan       = $dataCriteria[$_REQUEST[xid]][$ckey]["BUS_NERACA_TAHUNJALAN"];
         $monjalan         = $dataCriteria[$_REQUEST[xid]][$ckey]["BUS_NERACA_MONJALAN"];
         $nilaiawal        = str_replace(",","",$dataCriteria[$_REQUEST[xid]][$ckey]["BUS_NERACA_VALUEAWAL"]);
         $nilaiakhir       = str_replace(",","",$dataCriteria[$_REQUEST[xid]][$ckey]["BUS_NERACA_VALUEAKHIR"]);
         $nilaijalan       = str_replace(",","",$dataCriteria[$_REQUEST[xid]][$ckey]["BUS_NERACA_VALUEJALAN"]);
         #print "####".$ckey.$cvalue."##".$tahunawal."<br>";
         #print $dataCriteria[$_REQUEST[xid]][53]["BUS_NERACA_TAHUNAWAL"];
         $id       = getId("BUS_NERACA","ID_BUS_NERACA");
         #print count($tahunawal[$ckey])."#".$ckey."<br>";
         if(count($tahunawal[$ckey])=="0"){
           $sql      = " INSERT INTO BUS_NERACA(ID_BUS_NERACA,ID_BUS_APPLICATION,ID_NERACA,BUS_NERACA_TAHUNAWAL,BUS_NERACA_TAHUNAKHIR, BUS_NERACA_TAHUNJALAN, BUS_NERACA_VALUEAWAL,BUS_NERACA_VALUEAKHIR,BUS_NERACA_VALUEJALAN,BUS_NERACA_KETAWAL,BUS_NERACA_KETAKHIR,BUS_NERACA_KETJALAN, BUS_NERACA_MONJALAN)
                         VALUES('$id','$_REQUEST[xid]','$ckey','".$_REQUEST["tahunawal"]."','".$_REQUEST["tahunakhir"]."','".$_REQUEST["tahunjalan"]."','".str_replace(",","",$_REQUEST["nilaiawal_".$ckey])."','".str_replace(",","",$_REQUEST["nilaiakhir_".$ckey])."','".str_replace(",","",$_REQUEST["nilaijalan_".$ckey])."','".$_REQUEST["ketawal"]."','".$_REQUEST["ketakhir"]."','".$_REQUEST["ketjalan"]."','".$_REQUEST["monjalan"]."')";
 ...
 function  get_frmneraca_save($cmd){
    global  $_NeracaType, $msgDBSave, $msgDBError;
    $app          = module_getApplication_byid($_REQUEST[xid]);
    $criteria     = gen_NeracaTypeByScoring($app[ID_SCORING_TYPE]);
    $dataCriteria = get_NeracaByApp($_REQUEST[xid]);
 ...
 function  get_frmneraca_update_($cmd){
    global  $_NeracaType, $msgDBUpdate;
    $app      = module_getApplication_byid($_REQUEST[xid]);
    $criteria = gen_NeracaTypeByScoring($app[ID_SCORING_TYPE]);
    foreach($criteria as $cparents=>$vparents){
      foreach($vparents as $ckey=>$cvalue){
         $sql = " UPDATE BUS_NERACA SET BUS_NERACA_TAHUNAWAL  ='".$_REQUEST["tahunawal"]."',
                                        BUS_NERACA_TAHUNAKHIR ='".$_REQUEST["tahunakhir"]."',
                                        BUS_NERACA_TAHUNJALAN ='".$_REQUEST["tahunjalan"]."',
                                        BUS_NERACA_VALUEAWAL  ='".str_replace(",","",$_REQUEST["nilaiawal_".$ckey])."',
                                        BUS_NERACA_VALUEAKHIR ='".str_replace(",","",$_REQUEST["nilaiakhir_".$ckey])."',
                                        BUS_NERACA_VALUEJALAN ='".str_replace(",","",$_REQUEST["nilaijalan_".$ckey])."',
                                        BUS_NERACA_KETAWAL    ='".$_REQUEST["ketawal"]."',
                                        BUS_NERACA_KETAKHIR   ='".$_REQUEST["ketakhir"]."',
                                        BUS_NERACA_KETJALAN   ='".$_REQUEST["ketjalan"]."',
                                        BUS_NERACA_MONJALAN   ='".$_REQUEST["monjalan"]."'
                  WHERE ID_BUS_APPLICATION='$_REQUEST[xid]' AND ID_NERACA='$ckey'";
         #print $sql."<br>"."<br>"; //exit();
          GLOBAL $connectionInfo, $SQLHost;
          $conn = sqlsrv_connect( $SQLHost, $connectionInfo);
          $query = sqlsrv_query($conn,$sql);
 ...
 function  get_frmneraca_update($cmd){
    ...
    foreach($dt as $kdt=>$vdt){
       $sql = " UPDATE BUS_NERACA SET BUS_NERACA_TAHUNAWAL  ='".$vdt["TAWAL"]."',
 ...
 function get_ScoringApplication($cmd){
    global  $_FormulaType,$_NERACAKET, $_Month;
    $app          = module_getApplication_byid($_REQUEST[xid]);
    $bus_formula  = get_FormulaByScoring($app[ID_SCORING_TYPE]);
    $neraca       = get_NeracaValuebyAppYear($_REQUEST[xid]);
    $ketneraca    = get_NeracaKetbyAppYear($_REQUEST[xid]);
    #print_r($neraca);
...
/*
 ...
 function get_NeracaValuebyApp($app_id){
    $sql    = " SELECT * FROM BUS_NERACA WHERE ID_BUS_APPLICATION='$app_id'";
    GLOBAL $connectionInfo, $SQLHost;
    $conn = sqlsrv_connect( $SQLHost, $connectionInfo);
    $query = sqlsrv_query($conn,$sql);
    while($data=sqlsrv_fetch_array($query)){
 ...
 function get_NeracaValuebyAppYear($app_id){
    $sql    = " SELECT A.* FROM BUS_NERACA A
 ...
 function get_NeracaKetbyAppYear($app_id){
    $sql    = " SELECT A.* FROM BUS_NERACA A
                LEFT JOIN TYPE_NERACA B ON B.ID_NERACA=A.ID_NERACA
                WHERE A.ID_BUS_APPLICATION='$app_id' AND B.STATUS<>'99'";
    #print $sql;
...
//penting jangan dihapus
 function module_hasilscoring($cmd){
       ...
       foreach($hsl_akhir as $hsl_idscoringpoint){
          foreach($hsl_idscoringpoint as $hsl){
              ...
              if((trim($hsl[NERACA])!="")){
                  ...
                  unset($param1);
                  unset($param2);
                  $sql          = " SELECT * FROM BUS_NERACA WHERE ID_NERACA='$hsl[NERACA]' AND ID_BUS_APPLICATION='$_REQUEST[xid]'";
                  #print $sql;
                  GLOBAL $connectionInfo, $SQLHost;
                  $conn = sqlsrv_connect( $SQLHost, $connectionInfo);
                  $query = sqlsrv_query($conn,$sql);
                  ...
                  unset($param1);
                  unset($param2);
                  $sql          = " SELECT $hsl[APPLICATION] AS NILAI FROM BUS_APPLICATION WHERE ID_BUS_APPLICATION='$_REQUEST[xid]'";
                  GLOBAL $connectionInfo, $SQLHost;
                  $conn = sqlsrv_connect( $SQLHost, $connectionInfo);
                  $query = sqlsrv_query($conn,$sql);
                  $data         = sqlsrv_fetch_array($query);
                 ...
                 # print $hasil_formula."<br>";
                  ...
                  if($rawal!="" && $rakhir!=""){
                    ...
                    $sql    = " SELECT SUM(TC.COLL_SCOR) AS JML_POINT FROM BUS_COLLATERAL BC
      ...
      foreach($rumus_final as $id_finformula=>$vbusfinformula){
          ...
          //print $hasil_finformula."#".$finrumus[$id_finformula];
          //$update_finscore                 = update_finscore($hasil_finformula,$_REQUEST[xid]);
          $hasil_finformula                = get_finscore($_REQUEST[xid]);
    ...
    $msg  .= "<div id=\"tablecontent\"><table width=\"100%\" cellspacing=\"0\" border=\"0\"><tr><td class=\"tablecontentgrid\" align=\"center\">Approval<br><br><br><br><br>(.............................................)</td><td class=\"tablecontentgrid\" align=\"center\">Verifikator<br><br><br><br><br>(.............................................)</td><td class=\"tablecontentgrid\" align=\"center\" width=\"300\">BBO/RBO<br><br><br><br><br>".$_USER[$app[APPLICATION_AONAME]]."</td></tr></table></div>";
 ...
 function get_min_scoringbyType($id_scoring_type){
    $sql    = " SELECT * FROM BUS_MIN_SCORING WHERE ID_SCORING_TYPE='$id_scoring_type' AND STATUS<>'99'";
    //$query  = mssql_query($sql);
    ...
    $query = sqlsrv_query($conn,$sql);
    $data  = sqlsrv_fetch_array($query);    
    return $data[MIN_SCORING_VALUE];
 }

 function update_finscore($value,$id_app){
    $select       = " SELECT * FROM BUS_APPLICATION WHERE ID_BUS_APPLICATION='$id_app'";
    GLOBAL $connectionInfo, $SQLHost;
    $conn = sqlsrv_connect( $SQLHost, $connectionInfo);
    $query = sqlsrv_query($conn,$sql);
 ...
 function get_finscore($id_app){
    $select       = " SELECT * FROM BUS_APPLICATION WHERE ID_BUS_APPLICATION='$id_app'";
    GLOBAL $connectionInfo, $SQLHost;
    $conn = sqlsrv_connect( $SQLHost, $connectionInfo);
    $query_select = sqlsrv_query($conn,$sql);
    $data         = sqlsrv_fetch_array($query_select);
    return $data[APPLICATION_FINSCORE];
 }
