<?php
  function daftar_scoringneraca($cmd){
     global $_STATUS, $COOK_USER_ID, $_MAX_REC_PER_PAGE, $_ScoringType, $_NeracaType;
     ...
     $tgrid->set_content(array("<b>Laporan Keuangan by Scoring</b>"));
     $isi .= $tgrid->build();
     
     $where .= " AND STATUS <> 99";       
     $sql    = " SELECT * FROM SCORING_NERACA WHERE 1=1 $where";
     ...
     $sql_pagging = " SELECT TOP $_MAX_REC_PER_PAGE * FROM SCORING_NERACA WHERE 1=1 $where AND ID_SC_NERACA NOT IN(SELECT  TOP $curr_page  ID_SC_NERACA FROM SCORING_NERACA WHERE 1=1 $where)";
     #print $sql_pagging; 
     ...
     $tgrid->set_header(array("NO","SCORING TYPE","PARAMETER","&nbsp;"));
     ...
     while($data=sqlsrv_fetch_array($query_pagging)){
        ...
        $delete= "<a href=\"index.php?cmd=".$cmd."_delete&xid=".$data[ID_SC_NERACA]."\"><img src=\"includes/page_main/img/del_lt_on.gif\" onclick=\"return confirmDelete();\" border=\"0\"></a>";
        $tgrid->set_content_align(array("center","left","left","center")); 
        $tgrid->set_content_width(array("5","","","30"));       
        $tgrid->set_content(array("$x",$_ScoringType[$data[ID_SCORING_TYPE]]."&nbsp;",$_NeracaType[$data[ID_NERACA]]."&nbsp;",$edit."&nbsp;".$delete));
        $x++;
     }
     ...
     return  Template_KotakPolos("Laporan Keuangan by Scoring List",$isi);   
  }
  
   function frm_scoringneraca($cmd){
      global  $_ScoringType, $_NeracaType;
      if($_REQUEST[xid]!=""){
      ...
      $row_id = 1;
      $cf = new cform();
      $cf->start($name="scoringneraca",$method="POST",array("action"		=> "index.php",
                                      									  "target" 		=> "_self",
      ...
      $input	 = inputSelect($name="criteriatype",$value=$_NeracaType,$criteriatype,$ext=array("onkeypress"=> "return disableEnter(this,event);"));
      $cf->set_row($row_id,$ext=array("class"	=> "tablecontent"));
      $cf->set_col_header($row_id,$text="Parameter",$ext=array("width"=>"150","class"	=> "tablecontent","valign"=>"top"));
      $cf->set_col_content($row_id,$input,$ext=array("valign"=>"top"));
      $row_id++;      
      ...
      $form .= $cf->finishnew();                                     									
      $form .= "</div>";
      return Template_KotakPolos("Laporan Keuangan by Scoring",$form);    
  }   
  
  function save_scoringneraca($cmd){
      global  $msgDBSave,  $msgDBError;
      $scoringtype          = $_REQUEST[scoringtype];
      $criteriatype          = $_REQUEST[criteriatype];
      $id            = getId("SCORING_NERACA","ID_SC_NERACA");
      $sql           = "  INSERT INTO SCORING_NERACA(ID_SC_NERACA,ID_NERACA,ID_SCORING_TYPE,STATUS)
      ...
      $query = sqlsrv_query($conn,$sql);
      if($query){
         return Template_KotakPolos("Laporan Keuangan by Scoring",$msgDBSave);
      }else{
         return Template_KotakPolos("Laporan Keuangan by Scoring",$msgDBError);
      }                        
  }    
?>